...
import type { Amount, PoolId, ShadeRoutePoolEssential, ShadeRoutePoolEssentialsIdMap } from "@/utils/types";

const getTokenDecimals = (s: any) => 6;

export default class ShadeCalc {
  private readonly routePairsById: Record<string, ShadeRoutePoolEssential>;
  ...
  calculatePathQuotaByEnding({ endingTokenAmount: endingTokenAmount, endingTokenId: endingTokenId, path: path }) {
    const {
      ...
      (pathSegment, poolId) => {
        ...
        else throw Error("constant product rule swap parameter error");
        return {
          inputTokenId: u,
  ...
  public calculatePathOutcome({
    ...
    const pathReduceResult = path.reduce(
        (pathSegment, poolId) => {
          const {
          ...
          try {
            ...
            throw Error(
              `Invalid trade size ${e.message} at path=${poolId}. tradeSize=${otherTokenDenomAmount.toNumber()} ${
                poolPairInfo.stableParams ? `stable_params.price_ratio=${poolPairInfo.stableParams?.priceRatio}` : ""
              }`
            );
          }
          return {
