<template>
  <section>
    <header class="flex items-center justify-between">
      <h2 class="text-2xl text-black font-semibold p-0 m-0 mb-2.5 flex-1">Transactions</h2>
    </header>
        ...
        <td class="flex text-xs py-2">
          ...
          <div class="flex flex-col justify-between flex-1">
            <div class="font-medium">
              ...
              <span class="font-bold text-warning">{{ tx.ibc ? "IBC" : "" }}</span>
            </div>
          ...
          <div class="flex flex-col justify-between items-end">
            <div class="font-medium text-right text-xs text-gray-600 inline">
              ...
              >
                ...
                <IgntDenom :chain-id="amount.chainId" :denom="amount.denom ?? ''" />
              </span>
            </div>
            <div class="opacity-60">
              <template v-if="tx.type === 'received'"> from: {{ tx.sender }} </template>
    ...
    <div v-else class="text-left text-black opacity-75 text-md font-normal py-8">Transaction history is empty</div>
    <div
...
const txs = computed(() => {
  return transferTxs.value?.map((x: any) => normalizeTX(x)) ?? [];
});
const onShowMore = () => {
