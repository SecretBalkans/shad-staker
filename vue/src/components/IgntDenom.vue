<template>
  <span v-if="modifier === 'base'" :title="title">
    {{ shorten ? short : denom }}
  </span>
  <span v-else-if="modifier === 'path' /* && pathExtracted.length > 0*/" :title="title">
    <span> {{ chainIdNormalized }} </span>
    <!--    <span v-for="(channel, $index) in pathExtracted" :key="$index" class="ml-1.5">
      {{ channel }}
    </span>-->
  </span>
  <div v-else-if="modifier === 'avatar'" :class="[sizeClassObject, 'token-avatar']" :title="title">
    <img :src="props.icon" alt="..." />
    <img
      style="width: 0.9rem; margin-left: -0.65rem; margin-top: 1.15rem"
      v-if="chainIdNormalized === 'osmo'"
      :src="`${chainIdNormalized}.svg`"
      :alt="`on ${chainIdNormalized}`"
    />
  </div>
</template>
...
const chainIdLong = computed(() => {
  return props.chainId?.split("-")[0]
});
const chainIdNormalized = computed(() => {
  const part = chainIdLong.value;
  if (props.shorten) {
...
const short = computed(() => {
  ...
  if (denom.value?.length > 15) {
    ...
    return props.isSecret ? denom.value : denom.value?.replace(/^u/, "").toUpperCase().replace('SS', 'sS');
  }
});
...
const title = computed(() => {
  return `${short.value} on ${chainIdLong.value}`;
});
</script>
